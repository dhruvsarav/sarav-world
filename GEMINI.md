...
1. **Playground Hub Section Nomenclature (`/apps/`)**:
   - Section 1: `⚡ Financial & Retail Calculators Suite`
   - Section 2: `❤️ Close to Heart Ecosystem`
2. **Footer Distinction**:
   - **Consumer Retail Web Apps (All Calculators)**:
...
4. **Universal 4-Action Suite Across All Calculators**:
   ...
   - `💾 Save to History`: Client-side `localStorage` scenario persistence with timestamp, load, delete, and clear actions, accompanied by non-blocking toast feedback.
5. **Zero Blocking Alert/Prompt Modals & Toast Standard (Gold Price Estimator Benchmark)**:
   - **All 7 Calculators** must NEVER use blocking `alert()` or `prompt()` dialogs for copy or scenario saving.
...
6. **Strict Two-Family Color Palette Standard**:
   ...
   - **Family B & C — Financial Wealth & Engineering Suite (4 Apps Unified)**: `/apps/salary-planner/`, `/apps/retirement-planner/`, `/apps/fd-calculator/`, `/apps/rd-calculator/`, and `/apps/` (Playground Hub)
